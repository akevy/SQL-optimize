	   ...
	   $.post("/SQlOptimize/ChartServlet",
			   {
			     ...
			     type:$("#select1").val(),
			     dbid:$("#select2").val(),
			     instance_num:$("#select3").val(),
			     begin_time:$("#creattimeid1").val(),
			     end_time:$("#creattimeid2").val()
			   },
			   function(data,status){ 
     			var jsonData = eval(data);
				...
				alert(data);
			     ...
			     $.each(jsonData, function(index, objVal) { 
			   	  var ap = "<tr><td>"+objVal.snap_time+"</td><td>"+objVal.snap+"</td><td><a target=_blank href=/SQlOptimize/ChartSql?"+objVal.sql_id+">"+objVal.sql_id+"</a></td><td>"+objVal.elapsed_time+"</td><td>"+objVal.parse_call_avg+"</td><td>"
			   	 +objVal.buffer_get_avg+"</td><td>"+objVal.disk_read_avg+"</td><td>"+objVal.cpu_time_avg+"</td><td>"+objVal.sharable_mem_avg+"</td></tr>";
			    	 $("#table1").append(ap);
			    	/*  alert($("#select1").val()); */
			    	  switch($("#select1").val()){
			    	  case "SORTS_DELTA":
			    	 jsay.push(objVal.disk_read_avg);
			    	  break;
			    	  case "time_avg":
			    		  jsay.push(objVal.elapsed_time);
			    	  break;
			    	  case "parse_call_avg":
			    		  jsay.push(objVal.parse_call_avg);
			    	  break;
			    	  case "buffer_get_avg":
			    		  jsay.push(objVal.buffer_get_avg);
			    	  break;
			    	  case "disk_read_avg":
			    		  jsay.push(objVal.disk_read_avg);
			    	  break;
			    	  case "cpu_time_avg":
			    		  jsay.push(objVal.cpu_time_avg);
			    	  break;
			    	  case "sharable_mem_avg":
			    		  jsay.push(objVal.sharable_mem_avg);
			    	  break;
			    	  }
			    	 jsax.push(objVal.snap_time);
	                
	                });	

			     
			     myCharts(jsax,jsay,$("#select1").val());  
			   });
...
function myCharts(xline,yline,title){
	 // 路径配置
    require.config({
        paths:{ 
...
<select name="select1" id="select1">
	<option value="SORTS_DELTA">sorts</option>
	<option value="EXECUTIONS_DELTA">executions</option>
	<option value="time_avg">elapsed_time</option>
	<option value="parse_call_avg">parse_call</option>
	<option value="buffer_get_avg">buffer_get</option>
	<option value="disk_read_avg">disk_read</option>
	<option value="cpu_time_avg">cpu_time</option>
	<option value="sharable_mem_avg">sharable_mem</option>
</select>
<span>db_name:</span>
...
<span>开始时间:</span>
<input type="text" name="creattime" id="creattimeid1" size="20" value=''  onclick="setDate(this,'yyyy-MM-dd HH:mm')">&nbsp;
<span>结束时间：</span>
<input type="text" name="creattime" id="creattimeid2" size="20" value=''  onclick="setDate(this,'yyyy-MM-dd HH:mm')">&nbsp;
